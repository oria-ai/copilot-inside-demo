...
const VideoLesson = ({ videoUrl, videoTitle, lessonId, handleActivityComplete, onNext, lessonDisplayName, showChapters = true }: VideoLessonProps) => {
  ...
  const [vimeoVideoTitle, setVimeoVideoTitle] = useState<string>('');
  ...
  const getEmbedUrl = (url: string) => {
    ...
    const videoIdMatch = url.match(/(?:watch\?v=|youtu\.be\/)([\w-]+)/);
    if (videoIdMatch) {
      return `https://www.youtube.com/embed/${videoIdMatch[1]}`;
    }
    // Fallback for Vimeo or other URLs
    return url;
  };
  ...
  const loadVideoTitle = async (player: Player) => {
    try {
      const title = await player.getVideoTitle();
      console.log('Loaded video title from Vimeo:', title);
      setVimeoVideoTitle(title);
    } catch (error) {
      console.error('Error loading video title:', error);
      // Fallback to provided title if Vimeo title fails
      setVimeoVideoTitle(lessonDisplayName);
    }
  };
  ...
  useEffect(() => {
    ...
    const setupPlayer = async () => {
      try {
        ...
        // Load video title from Vimeo
        await loadVideoTitle(player);
  ...
  }, [videoUrl, lessonId, handleActivityComplete, progressSaved, showChapters, lessonDisplayName]);
  ...
  // Use Vimeo title if available, otherwise fallback to provided title
  const displayTitle = vimeoVideoTitle || lessonDisplayName;

  return (
    <div className="space-y-6">
      <Card className="bg-white rounded-3xl shadow-card">
        <CardHeader>
